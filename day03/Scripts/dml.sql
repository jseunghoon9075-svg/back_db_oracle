...
INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER)
VALUES(SEQ_PARENT.NEXTVAL, '장보고', 37, '경기도 군포시', '010-1234-5678', 'M');

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER)
VALUES(SEQ_PARENT.NEXTVAL, '김영희', 45, '경기도 안산시', '010-1234-5678', 'W');

INSERT INTO TBL_PARENT(ID, PARENT_NAME, PARENT_AGE, PARENT_ADDRESS, PARENT_PHONE, PARENT_GENDER)
VALUES(SEQ_PARENT.NEXTVAL, '김철수', 50, '서울시 관안구', '010-8813-6644', 'M');
...
SELECT SUM(FIELD_TRIP_NUMBER) AS "총을 든 인원수"
FROM TBL_FIELD_TRIP
WHERE FIELD_TRIP_TITLE LIKE '%체험%';
	

--	체험학습 제목에 '고구마' 또는 '아이스크림' 들어간 체험 학습의 평균 인원수
SELECT AVG(FIELD_TRIP_NUMBER) AS "평균 인원"
FROM TBL_FIELD_TRIP
...
--	2. 그룹 함수를 조회할 수 없다.
SELECT PRODUCT_NAME, FLOOR(AVG(PRODUCT_PRICE))
FROM TBL_PRODUCT
...
--	1) 가격대별 상품 재고 수
--	|| : 연결
SELECT PRODUCT_PRICE || '원' AS "상품 가격", FLOOR(AVG(PRODUCT_STOCK)) || '개' AS "평균 재고"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE
HAVING PRODUCT_PRICE = '1000' OR PRODUCT_PRICE = '2000' OR PRODUCT_PRICE = '3000' OR PRODUCT_PRICE = '4000' OR PRODUCT_PRICE = '5000';
--	2) 재고가 50개 이상의 개수
SELECT COUNT(PRODUCT_STOCK) || '개'
FROM TBL_PRODUCT
...
--	가격이 1000, 5000, 3000 인 상품만 조회

SELECT PRODUCT_NAME AS 상품명, PRODUCT_PRICE AS "상품 가격", PRODUCT_STOCK AS 재고
FROM TBL_PRODUCT 
WHERE PRODUCT_PRICE IN ('1000', '5000', '3000');

--	상품 재고가 15, 20, 25 인 상품 중
--	상품 재고별 개수와 평균 가격 구하기

SELECT 
	PRODUCT_STOCK || '개' AS "재고별 개수", 
	CEIL(AVG(PRODUCT_PRICE)) || '원' AS "평균 가격"
FROM TBL_PRODUCT
WHERE PRODUCT_STOCK IN ('15', '20', '25')
GROUP BY PRODUCT_STOCK;

--	모든 상품의 재고와 가격의 합
SELECT SUM(PRODUCT_PRICE) + SUM(PRODUCT_STOCK) AS "모든 상품의 재고와 가격의 합"
FROM TBL_PRODUCT;
--	상품 가격대별 기대수익
SELECT 
	PRODUCT_PRICE || '원' AS "개당 가격", 
	SUM(PRODUCT_STOCK) || '개' AS "재고" ,
	PRODUCT_PRICE * SUM(PRODUCT_STOCK) || '원' AS "기대수익"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE;
...
SELECT PRODUCT_PRICE || '원' AS "가격", FLOOR(10000 / PRODUCT_PRICE) || '개' AS "10000원으로 살 수 있는 갯수"
FROM TBL_PRODUCT
GROUP BY PRODUCT_PRICE;
