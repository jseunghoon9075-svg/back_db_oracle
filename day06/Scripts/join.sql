...
SELECT FLOOR(AVG(PRODUCT_PRICE)) AS "평균 가격"
FROM TBL_ORDER TBO
JOIN TBL_PRODUCT TBP
ON TBO.PRODUCT_ID = TBP.ID
JOIN TBL_BUYER TBB 
...
SELECT TBB.BUYER_NAME , TBB.BUYER_ADDRESS 
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB 
ON TBO.BUYER_ID = TBB.ID
WHERE TBB.BUYER_AGE >= 30 AND TBB.BUYER_GENDER = '남'
GROUP BY TBB.BUYER_NAME, TBB.BUYER_ADDRESS;
...
SELECT TBB.BUYER_NAME , TBB.BUYER_PHONE 
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB
ON TBO.BUYER_ID = TBB.ID
WHERE TBB.BUYER_ADDRESS LIKE '%서울시 강북구%';
...
SELECT SUM(PRODUCT_PRICE) AS "상품 가격 총합"
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB
ON TBO.BUYER_ID = TBB.ID
JOIN TBL_PRODUCT TBP 
...
-- 28) 상품별 기대 매출 조회
SELECT PRODUCT_NAME AS "상품명", PRODUCT_PRICE * PRODUCT_STOCK AS "기대 매출"
FROM TBL_PRODUCT;

-- 29) '유리' 구매자의 주문 내역 중 가장 최근 주문한 상품의 이름과 주문일자를 조회
SELECT TBB.BUYER_NAME AS "구매자", TBP.PRODUCT_NAME AS "상품명", TBO.ORDER_START_DATE AS "주문일자"
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB
ON TBO.BUYER_ID = TBB.ID
JOIN TBL_PRODUCT TBP 
...
SELECT TBB.ID, TBB.BUYER_NAME, TBO.ORDER_START_DATE
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB
ON TBO.BUYER_ID = TBB.ID
GROUP BY TBB.ID, TBB.BUYER_NAME, TBO.ORDER_START_DATE
ORDER BY ORDER_START_DATE ASC;

-- 31) '스웨이드' 브랜드의 모든 상품 주문 건수를 조회
SELECT COUNT(TBP.ID) AS "주문 건수"
FROM TBL_ORDER TBO
JOIN TBL_PRODUCT TBP 
ON TBO.PRODUCT_ID = TBP.ID
...
SELECT TBO.BUYER_NAME, TBP.PRODUCT_NAME, TBP.PRODUCT_PRICE
FROM (
	SELECT *
	FROM TBL_ORDER TBO
	JOIN TBL_BUYER TBB
	ON TBO.BUYER_ID = TBB.ID
	WHERE TBB.BUYER_NAME = '봉미선'
)TBO
JOIN TBL_PRODUCT TBP 
ON TBP.ID = TBO.PRODUCT_ID
...
SELECT COUNT(TBB.BUYER_NAME) AS "주문 건수", SUM(TBP.PRODUCT_PRICE) AS "가격 총합"
FROM (
	SELECT *
	FROM TBL_ORDER TBO
	JOIN TBL_BUYER TBB
...
SELECT TBB.BUYER_NAME AS "구매자", COUNT(TBB.BUYER_NAME) AS "주문한 상품 개수"
FROM TBL_ORDER TBO
JOIN TBL_BUYER TBB
ON TBO.BUYER_ID = TBB.ID
GROUP BY TBB.BUYER_NAME;
...
SELECT MAX(TBP.PRODUCT_PRICE) AS "주문 상품중 가장 비싼상품"
FROM (
	SELECT *
	FROM TBL_ORDER TBO
	JOIN TBL_BUYER TBB
...
WHERE TBB.BUYER_ID != TBO.BUYER_ID;

-- 18) PRODUCT_NAME이 '텀블러'인 상품을 TBL_PRODUCT 테이블에서 삭제
DELETE
FROM TBL_ORDER WHERE PRODUCT_ID = 10;

DELETE
FROM TBL_PRODUCT WHERE PRODUCT_NAME = '텀블러';
SELECT *
FROM TBL_PRODUCT;
...
CREATE TABLE TBL_INCOME(
	...
	INCOME NUMBER,
	INCOME_DAATE TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
	EMPLOYEE_ID NUMBER,
...
ON TI.EMPLOYEE_ID = TE.ID;                                                               MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM